...
def test_route_parser_exposes_stages_and_aggressive_mesh_defaults():
    ...
    segment = parser.parse_args(["segment", "output", "--prompts", "vehicle"])
    track = parser.parse_args(["track", "output", "--overwrite"])
    segment_unlimited = parser.parse_args(
        ["segment", "output", "--prompts", "vehicle", "--no-max-mesh-range"]
    )
    ...
    surface_segment = parser.parse_args(
        ["surface", "segment", "surface-output", "--prompts", "gravel carriageway"]
    )
    ...
    assert extract.sam3_recovery_distance_m == 1.0
    ...
    assert segment.adaptive_sam3_recovery is True
    assert segment.sam3_recovery_rounds == 2
    ...
    assert reconstruct.max_reconstruction_view_attempts == 3
    ...
    with pytest.raises(SystemExit):
        parser.parse_args(["segment", "output", "--prompt", "vehicle"])
